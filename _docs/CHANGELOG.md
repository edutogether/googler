...
## 2026-09-10 종합감사 (②단계, 대표 지시 순서의 2번째)

**점수 기준: (A)트랙 — "지금 만든 만큼 기준".** 준비 중인 월드가 많은 것(재배선 전 상태)은 미완성이지 결함이 아니므로 그걸로 감점하지 않았다. 2026-09-02 §7 종합감사에서 이미 10/10 확정된 항목은 §4-4에 따라 다시 파고들지 않았다 — 이번 라운드는 ①그 이후 실제로 바뀐 부분(도메인·공유 카드·문서 구조), ②이번에 처음 적용한 새 관점(§21)만 대상으로 했다.

**COMMON_STANDARDS §21(검사가 실제로 무언가를 봤는지) — 이 저장소에 처음 적용:**

- `.github/workflows/*.yml`의 `continue-on-error: true`(Firestore 규칙 배포)는 §21-1 빈 게이트가 **아님** — `steps.rules_deploy.outcome == 'failure'`를 실제로 확인해 `::error::` 어노테이션을 남기는 후속 스텝이 이미 있다(2026-09-02에 이 정확한 이유로 추가된 것). `filter().forEach(assert)` 패턴은 `src/`·`scripts/`에 0건.
- **결함 발견 — `scripts/visual-regression.mjs`**: `main()`이 `shots`(캡처된 스크린샷)의 개수를 한 번도 확인하지 않고 있었다. `VIEWPORTS`나 `PAGES` 배열이 어떤 이유로든 비게 되면(리팩터 실수 등) `results = []`, `failed.length = 0`이 되어 **"전부 통과: 0장 모두 기준과 일치합니다"로 초록불이 뜬다** — Poster Studio의 charset 검사와 같은 병. **고침**: 캡처 전 `VIEWPORTS.length === 0 || PAGES.length === 0`을 막고, 캡처 후 `shots.size`가 배열에서 유도한 값이 아니라 **독립적인 최소값(20)** 이상인지 확인하도록 수정 — 처음엔 `VIEWPORTS.length * PAGES.length`와 비교하는 방식으로 고쳤다가, 그 값 자체가 배열이 비면 같이 0이 되는 **동어반복**(§21-1이 정확히 경고하는 두 번째 형태)이라는 걸 재검토 중 발견해 독립 상수로 바꿨다. §21-2대로 `PAGES = []`로 강제해 실제로 빨간불(종료코드 1)이 뜨는 것 확인 후, 원복해 20/20 정상 통과까지 재확인.
- `src/data/firebase/firestore.rules.test.ts`는 `it()` 8개가 전부 개별 작성돼 있어 배열 순회로 테스트를 만드는 곳이 없음 — 이 유형의 결함 없음.

**실제 사고(2026-09-10, 감사 도중 발견) — CI 테스트 플레이크:**

문서 정리 커밋(`52190bb`) push 시 `ci.yml`(Verify pull requests)이 실패했다 — 같은 커밋의 `firebase-hosting-merge.yml`은 성공(같은 `npm run test:run` 스텝인데 결과가 갈림 → 결정적 회귀가 아니라 타이밍 경합). 원인: `MainWorldV3.test.tsx`의 "shows the approved planner visual scene..." 테스트가 `.mw3-shell` 클릭 직후 씬 전환 리전이 나타났다고 **동기적으로** 단언하고 있었다(`waitFor` 없이) — 같은 테스트 안 다른 모든 전환 지점은 `waitFor`를 쓰는데 이 한 곳만 빠져 있었다. CI 로그에 실제로 `data-transition="loading"`에 멈춘 DOM이 찍혀 있어 원인이 정확히 이거였다는 걸 확인했다(CLAUDE.md에 이미 기록된 "jsdom 30에서 노출된 것과 같은 계열의 React 19 타이밍 취약성"과 같은 부류지만, 이번엔 현재 jsdom 버전에서도 간헐적으로 발생한 것 — 새로 확인된 사실이라 §4-4 재감사 제한 대상이 아니다). **고침**: 그 한 줄을 `waitFor(..., { timeout: 2000 })`로 감쌌다(같은 파일의 다른 8곳과 동일한 패턴). 로컬에서 5회 연속 통과 확인.

**재채점(§4-2, 발견한 결함을 전부 고친 뒤)**: 위 두 건 모두 고쳤고 §21 관점을 포함해 다시 봤을 때 추가로 발견된 결함 없음 — **(A)트랙 기준 10/10.**

**감점하지 않은 것(§4-4 근거)**: 2026-09-02 확정 10/10 항목 10개는 재조사하지 않았다. 미완성 월드(재배선 전)는 의도된 설계라 감점 대상이 아니다.
...
## 2026-09-10 팀장 피드백 반영 — 같은 결함 두 번째 자리 + 태그 재발행

팀장이 Portal·Voice Cinema 완주 사례에서 나온 5가지 함정을 전달, 이 저장소에 같은 자리가 있는지 재점검.

**② 재점검 — 같은 결함이 실제로 하나 더 있었다.** "사고 뒤에 만든 테스트가 사고 난 자리를 보고 있는지" 확인하라는 지적에 따라, 방금 고친 `MainWorldV3.test.tsx` 줄 330 부근 패턴을 파일 전체에서 재검색 — **줄 300 부근, 퀘스트/도감/커뮤니티 3개 씬을 도는 for 루프 안에 완전히 같은 패턴**(`.mw3-shell` 클릭 직후 `waitFor` 없이 씬 전환 리전 등장을 동기 단언)이 하나 더 있었다. 이건 한 테스트 안에서 3번 반복되는 루프라 노출 표면이 더 컸다. 같은 방식(`waitFor`)으로 수정, 파일 전체를 재검색해 잔여 인스턴스 없음 확인, 단독 8회 연속 로컬 통과 확인.

**나머지 4가지 점검 결과(문제 없음 확인)**:
- 문서 분량: `wc -m`도 확인함(①단계에서 이미 실시) — CLAUDE.md 줄당 평균 155자, 극단적으로 긴 줄 없음(최대 757자, 기존 문서 관례 범위).
- §4-1-1 구조적 상한 재검토: 이 저장소 문서에 "구조적 상한이라 결함 아님"으로 명시적으로 적어둔 항목 자체가 없음(grep 0건). 가장 가까운 후보(예산 알림 메일-only, eslint/jsdom/typescript 보류)는 GCP 실제 제약·로컬 재현된 결정적 실패가 근거라 전제가 여전히 유효.
- vite preview 정적 서버 함정: `scripts/visual-regression.mjs`의 유일한 `fetch().ok` 사용처(`waitForServer`)는 서버 기동 여부만 확인하는 용도라 특정 파일 존재를 상태코드만으로 판정하는 곳이 아님 — 해당 없음.
- CI 명령 결과를 변수/출력에 담는 스텝: `.github/workflows/*.yml` 전체에 `$GITHUB_OUTPUT`/`$GITHUB_ENV`/커맨드 치환 패턴 0건.
...
## 2026-09-10 유실 문서 한 줄 복원 — reduced-motion과 시각 회귀의 관계
...
§23-3 경로 필터(`fbb3956`)의 양방향 중 "문서만 바뀌면 배포가 안 돈다" 한쪽은 커밋 `4c7213d`로 검증됨. **반대 방향("코드가 바뀌면 배포가 정상 실행된다")도 이후 확인됨** — 의미 없는 확인용 커밋을 따로 만들지 않고, Dependabot 대응(커밋 `338bf82`, package.json/package-lock.json 변경)이라는 실제 코드 커밋이 나갈 때 `firebase-hosting-merge.yml`이 정상 트리거·성공한 것으로 자연히 확인됨(§23-1 위반 없이). 양방향 전부 확인 완료.
...
## 2026-09-11 정기감사 주기 규칙화 (COMMON_STANDARDS §25)

대표님이 감사 주기를 확정: 정기 종합감사는 분기 말(3·6·9·12월 말, 여덟 저장소 공통)에 돈다. 개발 중엔 필요할 때 수시로 돌되 세션이 먼저 제안하지 않고(§19), 종합감사 10/10 확정이 "개발을 그만해도 된다"는 신호다. `CLAUDE.md`의 날짜 고정 문구("2026-09-30까지")를 규칙 형태로 바꿨다 — 날짜만 적으면 그날이 지나면 낡으므로, 다음 정기감사일(2026-09-30)과 그 이후에도 반복 적용되는 규칙을 같이 적었다.

## 2026-09-11 부팅 스플래시 — 최소 두 바퀴 하한 적용 (COMMON_STANDARDS §27)

대표님 지시: 모든 앱의 스플래시·로딩 화면은 자기 애니메이션이 최소 두 바퀴 도는 동안 떠 있어야 한다("초"가 아니라 "바퀴" 기준 — 애니메이션 주기가 바뀌면 하한도 같이 바뀐다). 이 앱은 기기별로 갈라진 스플래시 경로가 따로 없다(모바일·태블릿·PC 전부 `index.html`의 정적 마크업 + 인라인 `<style>` 하나) — 그래서 AI Ways Incheon처럼 "모바일에만 넣었다가 PC가 스쳐 지나가는" 형태의 결함은 구조적으로 없지만, 실제 hold 시간이 두 바퀴 기준을 만족하는지는 확인이 안 돼 있었다.

**결함 발견**: 로딩바 `.sbar i`의 `splashBar` 애니메이션은 1.15s/바퀴라 두 바퀴 = 2300ms인데, 기존 hold는 1800ms — 빠른 조건에서는 로딩바가 약 1.57바퀴만 돌고 페이드가 시작되고 있었다. `index.html`의 `#splash` 애니메이션 총 길이를 2400ms → **2900ms**로 늘리고(유지 2300ms → 페이드 500ms → 소멸), 그만큼 `@keyframes splashOut`의 정지점(%) 값을 다시 계산해 반영했다. 로드 게이트 상한(`src/splash.ts`의 `MAX_VISIBLE_MS = 4000`)은 그대로 뒀다(장애 대비 상한은 손대지 않는다는 지시대로).

**검사로 못박음**: `src/splash.test.ts`에 `index.html`을 직접 읽어 `splashBar` 주기와 `splashOut` hold 시간을 파싱한 뒤 `hold >= 2 * loop`를 검증하는 테스트를 추가 — 하드코딩된 밀리초가 아니라 실제 CSS 값에서 계산하므로, 나중에 로딩바 주기를 바꾸면 하한도 따라간다. **일부러 hold를 1800ms(구값)로 낮춰 실제로 빨간불(`expected 2175 to be greater than or equal to 2300`)이 뜨는 것 확인 후 원복.**

**실측**: Playwright + 실제 Chrome으로 모바일(390×844)·태블릿(850×1200)·PC(1920×1080) 세 뷰포트를 스로틀 없이 재방문 조건으로 각각 재서, 스플래시 등장부터 DOM에서 사라지기까지 걸린 시간을 측정 — 모바일 3018ms, 태블릿 2901ms, PC 2901ms. 셋 다 설계값(2900ms)과 거의 정확히 일치하고 두 바퀴 하한(2300ms)을 넉넉히 넘긴다.

**낡은 서술 정정**: 겸사겸사 발견한 것 — `index.html` 본문 주석과 `scripts/visual-regression.mjs`의 주석이 이미 존재하지 않는 `src/styles/splash.css`를 계속 가리키고 있었다(2026-09-09 표준 전환 때 CSS를 인라인으로 옮기며 놓친 자리). 둘 다 정정. `AGENTS.md`·`_docs/intents/2026-09-09-splash-logo-asset/intent.md`의 옛 타이밍 수치(1800/2400ms)도 새 값으로 갱신.

**검증**: `npm run check`(typecheck/lint/test 122개/build) + `npm run visual` 20/20(reduced-motion이 애니메이션을 1ms로 접어서 기준 이미지엔 영향 없음, 확인됨).

프리즈 태그는 다시 찍지 않음(팀장 지시대로 이런 눈에 보이는 작은 수정은 CHANGELOG에 남기고 다음 라운드에 함께 태그).

## 2026-09-11 부팅 스플래시 안전판 — org 공통 처방이 이 앱엔 안 맞는 것으로 판명

팀장이 §27 후속으로 "번들 안에 안전판이 있으면 번들이 안 올 때 상한이 작동 안 한다"(Poster Studio·Codyssey에서 실제 확인된 결함)를 org 전체에 경고, 이 저장소도 같은지 확인 요청.

**처음엔 같은 결함으로 보였다** — JS 번들 요청을 8초 지연시킨 뒤 `document.getElementById('splash')`로 판정했더니 8038ms까지 DOM에 남아 있었다.

**하지만 판정 방법 자체가 틀렸다는 걸 팀장 지적으로 알아챘다.** `abort()`가 아니라 **진짜로 응답을 영원히 멈추는 방식**(route 핸들러가 절대 resolve 안 하는 Promise 반환)으로 다시 재고, **DOM 존재 여부가 아니라 실제 `opacity`·`pointer-events`를 500ms마다 12초간 샘플링**했더니 — 번들이 12초 넘게 전혀 안 와도 **약 3.1초에 `opacity:0`·`pointer-events:none`이 되고 그 상태를 계속 유지**했다.

**원인**: 이 앱의 `#splash` 페이드는 `animation: splashOut 2900ms ease forwards`라는 **순수 CSS 애니메이션**이라, JS가 한 줄도 안 돌아도 브라우저가 페인트하는 순간 스스로 재생된다. `installSplash()`는 "끝난 노드를 지우는 일"과 "로드가 안 끝났으면 유지를 연장하는 일"만 하고, **사라짐(페이드) 자체는 JS에 의존하지 않는다.** Poster Studio·Codyssey는 JS가 클래스를 붙여야 페이드가 시작되는 구조라 그 안전판을 번들 밖으로 빼야 했지만, 이 앱에 같은 처방을 적용하면 **실제로는 없는 문제를 고치는 것**이 된다 — 그래서 고치지 않기로 팀장과 합의.
