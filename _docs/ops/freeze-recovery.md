...
**2026-09-10 정리 이후, 오래된 지점(현재 프리즈 최신 태그가 아닌 것)으로 돌아가야 하면 `refs/archive/tags/`를 본다** — 전체 목록·성격 구분(복구용/대조용)은 `_docs/archive/tags-20260910.md`. **새로 클론한 곳에서는 먼저 `git fetch origin 'refs/archive/*:refs/archive/*'`를 실행해야 그 참조들이 보인다** — 기본 fetch refspec(`refs/heads/*`)에 안 걸려 있어서 이 한 줄 없이는 "커밋이 그대로 살아있다"는 게 실제로 확인이 안 된다.
