...
export default function JourneyPrototypeRefined() {
  ...
  const [sound, setSound] = useState(() => localStorage.getItem('journey-sound') !== 'off');
  ...
  const shell = (children: React.ReactNode) => <main className="min-h-screen overflow-x-hidden bg-[#f8f9fa] px-4 py-6"><div className="mx-auto max-w-6xl"><header className="mb-6 flex items-center justify-between gap-3"><div><p className="text-xs font-bold text-[#5f6368]">여정 준비 {stage} / 8</p><p className="text-sm font-bold text-[#1a73e8]">{stageNames[stage - 1]}</p></div><button onClick={() => setSound(!sound)} className="shrink-0 rounded-full border border-[#dadce0] bg-white px-4 py-2 text-sm text-[#202124]">{sound ? '🔊 사운드' : '🔇 음소거'}</button></header><div className="mb-8 h-1.5 overflow-hidden rounded-full bg-[#e8eaed]"><div className="h-full bg-[#1a73e8]" style={{ width: `${stage / 8 * 100}%` }} /></div>{children}</div></main>;
  if (mode === 'legacy') return <LegacyGooglerApp />;
  if (mode === 'new') return shell(<IdentityStep onContinue={() => { play(); setMode('diagnostic'); }} />);
  if (mode === 'entry') return shell(<section className="mx-auto max-w-4xl py-10"><p className="font-bold text-[#1a73e8]">Google Educator journey</p><h1 className="mt-6 text-5xl font-black text-[#202124]">Be a Googler</h1><p className="mt-5 text-xl text-[#5f6368]">배우고 직접 해보며 함께 성장하는 구글러의 여정을 시작해보세요.</p><div className="mt-12 grid gap-5 md:grid-cols-2">{[['처음 여정 시작하기', '나에게 맞는 출발 단계를 찾아볼게요.', '🧭', 'new'], ['지난 여정 이어가기', '지금의 학습 여정으로 돌아갈게요.', '🎒', 'return']].map(([title, text, icon, target]) => <button key={title} onClick={() => { startJourneyBgm(sound); play(520); setMode(target as Mode); }} className="rounded-[2rem] border border-[#dadce0] bg-white p-8 text-left shadow-sm"><span className="text-4xl">{icon}</span><h2 className="mt-5 text-2xl font-black text-[#202124]">{title}</h2><p className="mt-2 text-[#5f6368]">{text}</p></button>)}</div></section>);
