...
export default function JourneyPrototypeRefined() {
  ...
  const shell = (children: React.ReactNode) => <main className={`journey-preview journey-preview--${mode} min-h-screen overflow-x-hidden bg-[#f8f9fa] px-4 py-6`}><div className="journey-preview__sky" aria-hidden="true" /><div className="journey-preview__frame mx-auto max-w-6xl"><header className="journey-preview__header mb-6 flex items-start justify-between gap-3"><div className="journey-preview__chapter"><p className="text-xs font-bold text-[#5f6368]">여정 준비 {stage} / 8</p><p className="text-sm font-bold text-[#1a73e8]">{stageNames[stage - 1]}</p></div><div className="journey-preview__controls flex max-w-[260px] flex-wrap justify-end gap-2 text-xs">{journey.history.length > 0 && <button type="button" aria-label="이전 단계" onClick={goBack} className="rounded-full border border-[#dadce0] bg-white px-3 py-2">이전 단계</button>}{mode !== 'entry' && <button type="button" onClick={restart} className="rounded-full border border-[#dadce0] bg-white px-3 py-2">처음부터 다시 시작</button>}<button aria-label="BGM 켜기 또는 끄기" onClick={() => { const next = saveAudioSettings({ bgmEnabled: !audioSettings.bgmEnabled }); setAudioSettings(next); if (!next.bgmEnabled) stopJourneyBgm(); else void startJourneyBgm(); }} className="rounded-full border border-[#dadce0] bg-white px-3 py-2">BGM {audioSettings.bgmEnabled ? '켜짐' : '꺼짐'}</button><button aria-label="효과음 켜기 또는 끄기" onClick={() => setAudioSettings(saveAudioSettings({ effectsEnabled: !audioSettings.effectsEnabled }))} className="rounded-full border border-[#dadce0] bg-white px-3 py-2">효과음 {audioSettings.effectsEnabled ? '켜짐' : '꺼짐'}</button></div></header><div className="journey-preview__route mb-8 h-1.5 overflow-hidden rounded-full bg-[#e8eaed]"><div className="h-full bg-[#1a73e8]" style={{ width: `${stage / 8 * 100}%` }} /></div>{children}</div></main>;
  ...
  if (mode === 'entry') return shell(<section className="journey-entry mx-auto max-w-4xl py-10"><div className="journey-entry__record"><p className="journey-entry__eyebrow font-bold text-[#1a73e8]">Google Educator journey</p><h1 className="journey-entry__title mt-6 text-5xl font-black text-[#202124]">Be a Googler</h1><p className="journey-entry__description mt-5 text-xl text-[#5f6368]">배우고, 직접 해보고, 함께 성장하는<br/>구글러의 작은 모험을 시작해보세요.</p><div className="journey-entry__choices mt-12 grid gap-5 md:grid-cols-2">{[['모험 시작하기', '나에게 맞는 출발 단계를 찾아볼게요.', '🧭', 'identity'], ['지난 모험 이어가기', '지금의 학습 여정으로 돌아갈게요.', '🎒', 'return']].map(([title, text, icon, target]) => <button key={title} onClick={() => { void startJourneyBgm(); void playJourneySfx(target === 'identity' ? 'journeyStart' : 'next'); navigate(target as JourneyStep); }} className={`journey-entry__choice rounded-[2rem] border border-[#dadce0] bg-white p-8 text-left shadow-sm ${target === 'identity' ? 'journey-entry__choice--primary' : ''}`}><span className="journey-entry__choice-icon text-4xl">{icon}</span><h2 className="mt-5 text-2xl font-black text-[#202124]">{title}</h2><p className="mt-2 text-[#5f6368]">{text}</p></button>)}</div></div></section>);
